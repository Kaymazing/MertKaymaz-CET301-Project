using MertKaymaz_301Project.Data;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Security.Cryptography;
using System.Text;

namespace MertKaymaz_301Project.Service
{
    public class AlaService
    {
        private AlaKurumsalDatabase db;
        public AlaService()
        {
            db = new AlaKurumsalDatabase();
        }

        public Owner Login(string Name, string Password)
        {
            string hashedPasword = hashPassword(Password);
            var loginUser = db.Owners.FirstOrDefault(u => u.Name == Name && u.Password == hashedPasword);

            return loginUser;
        }
        public bool CheckPassword(Owner user, string password)
        {
            return user.Password == hashPassword(password);
        }
        public string hashPassword(string plainPassword)
        {
            using (SHA256 sha256Hash = SHA256.Create())
            {
                var plainBytes = Encoding.UTF8.GetBytes(plainPassword);
                // ComputeHash - returns byte array  
                byte[] bytes = sha256Hash.ComputeHash(plainBytes);

                // Convert byte array to a string 
                StringBuilder builder = new StringBuilder();
                for (int i = 0; i < bytes.Length; i++)
                {
                    builder.Append(bytes[i].ToString("x2"));
                }
                return builder.ToString();

            }
        }



    }
}
